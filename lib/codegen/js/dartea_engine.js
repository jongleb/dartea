...
const $$instance = (form) => {
  ...
  root.$$block = { nodes: form.holes.map((hole) => $$located(root, hole.path)), held: [], lists: [] };
  return root;
};

const $$refresh = (element, vnode, dispatch, taggers) => {
  const { nodes, held, lists } = element.$$block;
  for (const [index, hole] of vnode.form.holes.entries()) {
    const value = vnode.values[index];
    ...
    if (hole.kind === "text") {
      node.data = value;
    } else if (hole.kind === "children") {
      const after = [...$$items(value)];
      $$children(node, lists[index] ?? [], after, dispatch, taggers);
      lists[index] = after;
    } else if (hole.kind === "subtree") {
      nodes[index] =
        before === undefined
          ? $$placed(node, $$element(value, dispatch, taggers))
          : $$patch(node.parentNode, node, before, value, dispatch, taggers);
    } else {
      $$attributes(node, before === undefined ? [] : [before], [value], dispatch, taggers);
    }
  }
};

const $$block = (vnode, dispatch, taggers) => {
  const element = $$instance(vnode.form);
  $$refresh(element, vnode, dispatch, taggers);
  return element;
};

const $$element = (vnode, dispatch, taggers) => {
  if (vnode.TAG === "block") return $$block(vnode, dispatch, taggers);
  if (vnode.TAG === "map") {
...
const $$patch = (parent, element, before, after, dispatch, taggers) => {
  ...
  if (after.TAG === "block") {
    $$refresh(element, after, dispatch, taggers);
    return element;
  }

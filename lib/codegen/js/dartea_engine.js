...
const $$aims = (b, root, type, happened) => {
  ...
  for (let index = 0; index < holes.length; index += 1) {
    ...
    const node = (b.nodes[index] ??= $$locate(root, hole.path));
...
const $$blueprint = (form) => {
  ...
  const made = {
    root,
    factory: form.holes.map((hole) => ($$fills(hole) ? $$locate(root, hole.path) : undefined)),
  };
  $$blueprints.set(form, made);
  return made;
};

const $$locate = (root, path) => {
  let node = root;
...
const $$apply = (b, index, value, before) => {
  ...
  const node = (b.nodes[index] ??= $$locate(b.root, hole.path));
  if (hole.kind === "text") {
    node.data = value;

...
const $$aims = (b, root, type, happened) => {
  ...
  for (let index = 0; index < holes.length; index += 1) {
    ...
    const node = (b.nodes[index] ??= hole.find(root));
...
const $$route = (app, happened) => {
  ...
  const cached = target.$$aim?.[type];
  if (cached !== undefined && cached.root.isConnected && cached.root.$$block === cached.b && cached.root.contains(target)) {
    for (const aim of cached.found) {
      if (!$$deliver(app, aim.node, aim, happened)) return;
    }
    for (let node = cached.root.parentNode; node !== null && node !== app.host; node = node.parentNode) {
...
const $$relist = (b, index, hole, node, value) => {
  const held = b.lists[index] ?? { fn: undefined, args: $$noentries, list: undefined, entries: $$noentries };
  const after = [];
  let fn;
  let args;
  let list;
  if (hole.kind === "children") {
    ...
    [fn, args, list] = value;
    const kept = fn === held.fn;
    const stable = kept && $$same(args, held.args);
    if (stable && list === held.list) return;
    if (kept && list === held.list) {
    ...
    for (let rest = list; rest !== 0; rest = rest.tl) {
      ...
      if (stable && positional !== undefined && positional.item === item) {
        ...
        if (stable && hole.keyed && positional !== undefined && positional.key === made[0]) {
          ...
          after.push(found ?? $$entry(true, at, item, made));
        } else {
          aligned = false;
    ...
    if (aligned && after.length === held.entries.length) {
      b.lists[index] = { fn, args, list, entries: held.entries };
      return;
    }
  }
...
const $$apply = (b, index, value, before) => {
  ...
  const node = (b.nodes[index] ??= hole.find(b.root));
  if (hole.kind === "text") {
    node.data = value;
...
const $$refresh = (element, vnode, app, taggers) => {
  ...
  b.app = app;
  b.taggers = taggers;
  vnode.refresh(b, $$put, vnode.args);
};
...
const $$block = (vnode, app, taggers) => {
  ...
  const b = {
    ...
    nodes: $$slots(count, null),
    held: $$slots(count, undefined),
    deps: $$slots(count, undefined),
    lists: $$slots(count, null),
    pending: undefined,
    app,
    taggers,
  };
  ...
  if (pending !== undefined) {
    for (const index of pending) $$apply(b, index, b.held[index], undefined);
  }
  return root;
};
...
const $$mount = (program, node, flags) => {
  ...
  const dispatch = async (msg) => {
    ...
    model = stepped;
    schedule();
    $$reconcile(program.watch(model), dispatch);

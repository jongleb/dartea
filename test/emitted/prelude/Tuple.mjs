...
const mapFirst = (func, $p1) => {
  const x$2 = $p1[0];
  const y$2 = $p1[1];
  return [Dartea_runtime.$$curry(func, [x$2]), y$2];
};
const mapSecond = (func$1, $p1$1) => {
  const x$3 = $p1$1[0];
  const y$3 = $p1$1[1];
  return [x$3, Dartea_runtime.$$curry(func$1, [y$3])];
};
const mapBoth = (funcA, funcB, $p2) => {
  const x$4 = $p2[0];
  const y$4 = $p2[1];
  return [Dartea_runtime.$$curry(funcA, [x$4]), Dartea_runtime.$$curry(funcB, [y$4])];
};
